...
def destripe_IBL(rec: si.SpikeGLXRecordingExtractor) -> Tuple[si.SpikeGLXRecordingExtractor, Tuple[np.ndarray, np.ndarray]]:
    ic('[**] IBL destriping [**] ')
...
def destripe_hybrid(rec: si.SpikeGLXRecordingExtractor) -> Tuple[si.SpikeGLXRecordingExtractor, Tuple[np.ndarray, np.ndarray]]:
...
def compare_processed(recordings: List[si.SpikeGLXRecordingExtractor], labels: List[str]):
    ...
    si.plot_traces({'{}'.format(labels[i]): recordings[i] for i in range(nplots)}, backend='matplotlib', mode='line',
                   ax=ax1, channel_ids=some_chans)
    savefig(f1, 'compare_processed_channels')


def run_preprocessing(ap_recording: si.SpikeGLXRecordingExtractor, lfp_recording: si.SpikeGLXRecordingExtractor,
                      save_path: Path) -> None:
...
def main_unprocessed():
    ...
    save_path = spikeglx_folder.joinpath('preprocessed')
    recordings = load_sglx_data(spikeglx_folder)
    ic(recordings)
    ...
    run_preprocessing(ap_recording=recordings[0], lfp_recording=recordings[1], save_path=save_path)
